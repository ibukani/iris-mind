...
## v0.2 変更点 — MemoryManager

v0.2 で新たに `iris.kernel.memory.MemoryManager` が追加され、記憶操作を一元管理します。

| メソッド | 説明 |
|---|---|
| `search_semantic(query, max_results=3)` | 意味記憶から関連エントリを全文検索（スコア付き） |
| `add_episodic(content, kind="user_input", metadata=None)` | エピソード記憶にエントリ追加 |
| `add_semantic(content, tags=None)` | 意味記憶に教訓・好みを追加 |
| `get_recent(n=3)` | 直近 n 件のエピソード記憶を時系列順に取得 |
| `get_user_preferences()` | ユーザーの好みを意味記憶から検索 |

### 記憶の分類（v0.2 変更）

| 記憶種別 | 管理先 | 変更点 |
|---|---|---|
| エピソード記憶 | `EpisodicStore` → `MemoryManager` 経由 | 直接アクセスではなく `MemoryManager` を経由 |
| 意味記憶 | `SemanticStore` → `MemoryManager` 経由 | 同上 |
| 構造記憶 | `iris_profile.md` | 変更なし |
| 手続き記憶 | 将来実装 | 変更なし |
