...
### 2. Phase 順: Protocol → Output 分離 → Input 分離

| Phase | 内容 | 理由 |
|-------|------|------|
| Phase 0 | EventBus Protocol 化 + イベントシリアライズ | 全 Phase の基盤。後方互換を維持したまま抽象化 |
| Phase 1 | Output Process 分離 | 最も独立しやすく効果が大きい（ステートレス、単方向通信） |
| Phase 2 | Input Process 分離 | Proactive 応答追跡を Kernel 側に移動してから実施 |
| Phase 3 | Controller プロセス導入 | ライフサイクル管理 |
| Phase 4 | マルチ入力ソース対応 | 複数 Input/Output の同時運用 |

### 3. Proactive 応答追跡の Kernel 側移動
...
- Phase 0 で ReplayableTransport + trace_id を導入してから分離開始
- Phase 0 完了時点で既存テスト全179件が通過することを確認
- 各 Phase で単一プロセスモード（--no-separate）を維持
- 移行中も従来の起動方法（単一プロセス）が動作し続ける
