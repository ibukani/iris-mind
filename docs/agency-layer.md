...
sequenceDiagram
    ...
    EX->>STM: short_term.add_turn("user", content, plan.account_id)  # Plan決定後に追加
    EX->>EX: LLMGateway.chat(plan, messages)
    EX->>EB: OutputRequest(stream)
    EX->>EB: OutputRequest(response)
    EX->>STM: short_term.add_turn("assistant", response, plan.account_id)
```
...
| `silent` | bool | サイレントモード（予約済み、現状実行時は無効） |
