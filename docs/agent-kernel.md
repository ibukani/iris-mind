...
3. **InputMessage 処理** — TcpListener からの入力を ConversationService/ProactiveEngine へ配送
...
EventBus は Kernel 内部専用。I/O は TcpListener (受信) / SessionManager.route_output (送信) で通信する。
...
[InputMessage 受信 (from TcpListener)]
...
[ProactiveEngine 発話決定]
    └── → AgentKernel.on_proactive_speech(content)
           ├── 発話をエピソード記憶に記録
            ├── SessionManager.route_output("", proactive, ...)
