...
| 層 | 責務 | 依存方向 |
|---|---|---|
| **adapters/** | ユーザーとの入出力（CLI, API, GUI） | → kernel |
| **kernel/** | ビジネスロジック全体 | → llm, memory, capabilities |
| **personality/** | システムプロンプト・人格シミュレーション | → memory |
| **memory/** | 記憶の永続化・検索・要約 | → 外部DB |
| **llm/** | Ollama等との通信 | → Ollama API |
| **capabilities/** | ツールの実行・動的発見 | → 外部ツール |
| **commands/** | CLIコマンドの解釈・実行 | → kernel |
...
- **イベント種別**: `UserInputEvent`, `ProactiveSpeechEvent`, `TimerTick`, `AgentStateChangeEvent`, `MemoryUpdateEvent`
