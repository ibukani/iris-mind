...
| Server → Client | `Message` (`chat`, `system`, `execute`, `ack`, `error`) | 応答、アクション要求、確認 |
| Server → Client | `Message`（`direction:stream`） | ストリーミング中継（state: thinking/speaking/done） |
...
| `state` | string | 任意 | null | ストリーム状態（`thinking`/`speaking`/`done`/`interrupted`） |
| `correlation_id` | string | 任意 | null | 応答連鎖の識別子 |
| `metadata` | object | 任意 | `{}` | 拡張メタデータ（自由なkey-value、実装固有の拡張に使用） |
...
| msg_type | 方向 | 必要な Permission |
|---------|------|-------------------|
| `chat` | Server→Client | `receive_chat` |
| `proactive` | Server→Client | `receive_chat` |
| `system` | Server→Client | `receive_chat` |
...
| `chat` | Client→Server | `send_chat` |
| `system` | Client→Server | `send_chat` |
| `interrupt` | 双方向 | `interrupt` |
