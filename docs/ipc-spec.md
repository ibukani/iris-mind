...
### 2.3 Proto 定義 (`proto/iris/io/iris_service.proto`)
...
| 通信方向 | 種別 | 説明 |
|---------|------|------|
| Client → Server | `ClientFrame.message` | テキスト入力、制御、アクション結果 |
| Client → Server | `ClientFrame.command` | システムコマンド（`CommandInput` fast-path） |
| Server → Client | `ServerFrame.message` | 応答、アクション要求、確認（`direction:stream`/`direction:response` で配送） |
| Server → Client | `ServerFrame.command` | コマンド応答（`CommandOutput` fast-path） |
...
sequenceDiagram
    ...
    Client->>Server: ClientFrame(Message: msg_type="chat", direction="request", content="Hello")
    Server->>Kernel: MessageEvent
    activate Kernel
    Kernel-->>Server: OutputRequest(thinking)
    Server-->>Client: ServerFrame(Message: msg_type="chat", direction="stream", state="thinking")
    Kernel-->>Server: OutputRequest(speaking, content="Hello")
    Server-->>Client: ServerFrame(Message: msg_type="chat", direction="stream", state="speaking", content="Hello!")
    Kernel-->>Server: OutputRequest(done)
    Server-->>Client: ServerFrame(Message: msg_type="chat", direction="stream", state="done")
    Kernel-->>Server: OutputRequest(response)
    Server-->>Client: ServerFrame(Message: msg_type="chat", direction="response", content="Hello! How can I help you today?")
    deactivate Kernel
    deactivate Server
```
...
sequenceDiagram
    ...
    Client->>Server: ClientFrame(CommandInput: content="/status")
    activate Server
    Server->>Kernel: Handle Command
    Note over Kernel: CommandHandler で直接処理
    Kernel-->>Server: CommandResult
    Server-->>Client: ServerFrame(CommandOutput: content="Status: OK")
    deactivate Server
```
...
def generate_messages(session_id):
    # 送信メッセージのイテレータ
...
def run():
    ...
    with grpc.insecure_channel('localhost:9876') as channel:
        ...
        # 最初はセッションIDが不明なので空で接続し、レスポンス内のセッションIDを取得するなどの処理を想定
        # 双方向ストリームの呼び出し
        responses = stub.BidirectionalStream(generate_messages(""), metadata=metadata)
...
```rust
use iris::iris_service_client::IrisServiceClient;
use iris::{ClientFrame, Message};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let permissions: MetadataValue<_> = "send_chat,receive_chat,send_command,receive_command".parse()?;
    ...
    let outbound = tokio_stream::iter(vec![
        ClientFrame {
            payload: Some(iris::client_frame::Payload::Message(Message {
    ...
    while let Some(frame) = inbound.message().await? {
        if let Some(payload) = frame.payload {
            match payload {
                iris::server_frame::Payload::Message(msg) => {
                    println!("Received message: {}", msg.content);
                }
                iris::server_frame::Payload::Command(cmd) => {
...
| 権限不足 | ServerFrame 内で `error` メッセージを送信、または `PERMISSION_DENIED` (7) |
