...
| Server → Client | `stream`, `response`, `command`, `proactive`, `ack` | 出力メッセージ |
...
| フィールド | 型 | 必須 | デフォルト | 説明 |
|-----------|-----|------|-----------|------|
| `msg_type` | string | 必須 | - | `"stream"`, `"response"`, `"command"`, `"proactive"`, `"ack"` |
| `state` | string | 任意 | - | stream の状態: `"thinking"`, `"speaking"`, `"done"` |
| `id` | string | 任意 | 自動生成 | メッセージID (12文字) |
...
| `/compact` | 会話履歴の圧縮 | `"Compacted: 240 chars summary..."` |
| `/clear` | 全記憶消去 | 内部応答 |
| `/reflect` | セッション振り返り | 内部応答 |
