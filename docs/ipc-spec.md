...
|--------|------|------|---------------|
| `account.join` | C→S | identity解決/作成、セッション/ルーム紐付け | `identity.provider`, `identity.subject` |
...
| `account.joined` | S→C | join完了応答 | - |
| `account.left` | S→C | leave完了応答 | - |
| `account.profile` | S→C | account.get応答 | - |
| `account.updated` | S→C | update完了応答 | - |
| `account.identity_linked` | S→C | link_identity完了応答 | - |
| `account.error` | S→C | account制御エラー | - |

**自動発行**:
- セッション切断時、サーバーは同一セッション配下の全ルーム参加者を退室扱いにする。
- アカウントのセッション/ルーム紐付け/解除時、サーバーは `presence.joined` / `presence.left` を `ControlMessage` として配信する。
...
sequenceDiagram
    ...
    Note over Client,Handler: 明示join
    Client->>Server: BidirectionalStreamRequest(ControlMessage: action="account.join", identity=discord:123)
    Server->>GW: on_grpc_control(control_msg, session_id)
    GW->>Handler: handle_control_message(msg, session_id)
    Handler->>Handler: publish(ControlMessageEvent) to EventBus
    ...
    Handler-->>GW: ControlMessage(action="account.joined", account_id="abc123", room_id, text="Joined: Bob")
    GW-->>Server: route_control_message
    Server-->>Client: BidirectionalStreamResponse(ControlMessage: action="account.joined", account_id="abc123", room_id)
