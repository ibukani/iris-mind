...
class RoomProvider:
    ...
    - ルームのCRUD
    - メンバーシップ管理（セッション追跡含む）
    - AccountProvider からのアカウント解決
    - EventBus へのイベント発行
    """

    def __init__(self, store: RoomStore, event_bus: Any = None, account_provider: Any = None) -> None:
        self._store = store
        self._event_bus = event_bus
        self._account_provider = account_provider

    def set_account_provider(self, account_provider: Any) -> None:
        self._account_provider = account_provider
    ...
    def update_room(self, room_id: str, **fields: Any) -> None:
        ...
        if not room:
            logger.warning("RoomProvider: room not found: {}", room_id)
            return
        ...
        logger.info("RoomProvider: updated room_id={}", room_id)
    ...
    def delete_room(self, room_id: str) -> None:
        ...
        logger.info("RoomProvider: deleted room_id={}", room_id)
    ...
    def join_room(self, room_id: str, account_id: str, session_id: str = "", role: str = "member") -> None:
        ...
        if existing and existing.room_id == room_id:
            logger.debug("RoomProvider: session {} already in room {}", session_id, room_id)
            return
        ...
        logger.debug("RoomProvider: account {} joined room {} (session={})", account_id, room_id, session_id)
    ...
    def leave_room(self, room_id: str, account_id: str, session_id: str = "") -> None:
        ...
        logger.debug("RoomProvider: account {} left room {} (session={})", account_id, room_id, session_id)
    ...
    def unbind_all_for_session(self, session_id: str) -> list[str]:
        ...
        logger.debug("RoomProvider: unbound all for session={}", session_id)
        return account_ids
    ...
    def get_account_by_session(self, session_id: str, room_id: str = "") -> Any | None:
        ...
        if member and (not room_id or member.room_id == room_id):
            return self._account_provider.resolve(member.account_id)
        return None
    ...
    def _resolve_nickname(self, account_id: str) -> str:
        ...
        if self._account_provider:
            account = self._account_provider.resolve(account_id)
            if account:
