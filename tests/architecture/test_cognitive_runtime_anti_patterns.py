"""Anti-pattern scans and feature boundary rules for v1.2.1 target architecture.

Rules enforced:
  1. No global mutable registries in new target modules.
  2. No service locator access patterns in new target modules.
  3. No EventBus as primary control-flow mechanism in cognitive/cycle.
  4. No PluginManager as primary extension mechanism in new target modules.
  5. No untyped dict[str, Any] public contracts in contracts/.
  6. No action: str dispatcher patterns in new target modules.
  7. No app-specific imports inside cognitive/presentation/safety/contracts/core.
  8. Features must use FeatureDefinition, not core orchestration.
  9. Features must not mutate WorkspaceFrame directly.
"""
...
# ── 3. EventBus in cognitive cycle ────────────────────────────


def test_no_eventbus_in_cognitive_cycle() -> None:
    """CognitiveCycle must not use EventBus as its main control flow."""
    cycle_dir = PROJECT_ROOT / "iris" / "cognitive" / "cycle"
    if not cycle_dir.is_dir():
        pytest.skip("iris/cognitive/cycle/ does not exist yet")

    violations: list[str] = []
    for filepath in _get_python_files(cycle_dir):
        rel = filepath.relative_to(PROJECT_ROOT).as_posix()
        for imp in _get_imports(filepath):
            if imp.startswith("iris.event") or "EventBus" in imp or "event_bus" in imp:
                violations.append(f"  {rel}: imports '{imp}' — cognitive/cycle must not use EventBus")
        try:
            text = filepath.read_text(encoding="utf-8")
        except Exception:  # noqa: S112
            continue
        if "EventBus" in text or ".subscribe(" in text:
            violations.append(f"  {rel}: references EventBus or .subscribe() — forbidden in cognitive/cycle")

    assert not violations, "EventBus usage in cognitive/cycle:\n" + "\n".join(violations)


# ── 4. PluginManager access ────────────────────────────────────


@pytest.mark.parametrize("target_dir", sorted(TARGET_DIRS))
def test_no_plugin_manager_in_target_modules(target_dir: str) -> None:
    """New v1.2.1 target modules must not use PluginManager as extension mechanism."""
    if not _target_exists(target_dir):
        pytest.skip(f"Target layer '{target_dir}' does not exist yet")
    ...
    for filepath in _get_python_files(base):
        rel = filepath.relative_to(PROJECT_ROOT).as_posix()
        for imp in _get_imports(filepath):
            if "kernel.manager" in imp or "PluginManager" in imp:
        ...
        if "PluginManager" in text:
            violations.append(f"  {rel}: references PluginManager")

    assert not violations, (
        f"PluginManager references found in {target_dir} — use constructor injection instead:\n" + "\n".join(violations)
    )


# ── 5. Untyped dict contracts ────────────────────────────────
...
# ── 7. App-specific imports in core layers ─────────────────────
