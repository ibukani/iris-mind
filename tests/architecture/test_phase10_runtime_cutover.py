"""Phase 12 runtime architecture guards — post-deletion.

Enforces:
  - main.py delegates to target runtime
  - Runtime files do not import legacy packages
  - Target packages are isolated from legacy
  - No PluginManager/EventBus in target path
  - Runtime CLI structure requirements
"""
                ...
                for alias in node.names:
                    for prefix in LEGACY_IMPORT_PREFIXES:
                        if alias.name.startswith(prefix):
                            ...
                            violations.append(f"{alias.name} from {rel}")
            elif isinstance(node, ast.ImportFrom) and node.module:
                for prefix in LEGACY_IMPORT_PREFIXES:
                    if node.module.startswith(prefix):
                        rel = py_file.relative_to(PROJECT_ROOT)
                        violations.append(f"{node.module} from {rel}")
...
def test_runtime_wiring_app_exists() -> None:
    assert (PROJECT_ROOT / "iris" / "runtime" / "wiring" / "app.py").is_file()
