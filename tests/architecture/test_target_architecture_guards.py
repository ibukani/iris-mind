"""Permanent target architecture guards for Phase 12.5+.

Enforces:
  1. Deleted legacy packages must not exist on disk or be importable.
  2. No legacy concepts (EventBus, PluginManager, etc.) in active source code.
  ...
  7. Active test suite is target-only (no legacy markers/dirs).
"""
...
DELETED_LEGACY_IMPORTS: set[str] = {
    "iris.account",
    "iris.agency",
    "iris.event",
    "iris.heartbeat",
...
# ═══════════════════════════════════════════════════════════════════
# 1.  Deleted legacy packages must not exist
# ═══════════════════════════════════════════════════════════════════
...
def test_deleted_legacy_packages_do_not_exist(pkg_dir: str) -> None:
    ...
    assert not pkg_path.exists(), f"Legacy package '{pkg_dir}' still exists"


@pytest.mark.parametrize("pkg_name", sorted(DELETED_LEGACY_IMPORTS))
def test_deleted_legacy_packages_not_importable(pkg_name: str) -> None:
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module(pkg_name)


# ═══════════════════════════════════════════════════════════════════
# 2.  No legacy concepts in active source code
# ═══════════════════════════════════════════════════════════════════
...
def test_no_forbidden_concepts_in_source_code(symbol: str) -> None:
    """Forbidden legacy concepts must not appear in active source code."""
    violations: list[str] = []
...
def test_no_legacy_imports_in_source_code() -> None:
    """Active source files must not import deleted legacy packages."""
    violations: list[str] = []
    for filepath in _iter_source_files():
        try:
        ...
        for imp in _all_imports(tree):
            for prefix in DELETED_LEGACY_IMPORTS:
                if imp.startswith(prefix):
                    rel = filepath.relative_to(PROJECT_ROOT).as_posix()
                    violations.append(f"  {rel}: imports '{imp}'")
    assert not violations, "Legacy imports found in source code:\n" + "\n".join(violations)
...
def test_entrypoint_no_legacy_imports(rel_path: str) -> None:
    """Entrypoint and wiring files must not import deleted legacy packages."""
    ...
    for prefix in DELETED_LEGACY_IMPORTS:
        if prefix.replace(".", "/") in text or prefix in text:
            ...
            tree = ast.parse(text)
            for imp in _all_imports(tree):
                assert not imp.startswith(prefix), f"{rel_path} imports legacy '{imp}'"
...
HEAVY_INIT_FORBIDDEN: set[str] = {
    ...
    "iris.runtime",
    "iris.features",
} | DELETED_LEGACY_IMPORTS


def test_init_files_no_heavy_imports() -> None:
    """Target package __init__.py must not import heavy/deleted/legacy modules."""
    violations: list[str] = []
        ...
        for imp in _all_imports(tree):
            for forbidden in HEAVY_INIT_FORBIDDEN:
                if imp.startswith(forbidden):
                    ...
                    violations.append(f"  {rel}: imports '{imp}'")
    assert not violations, "__init__.py files must not import heavy or legacy modules:\n" + "\n".join(violations)
...
# ═══════════════════════════════════════════════════════════════════
# 7.  Active test suite is target-only
# ═══════════════════════════════════════════════════════════════════
...
LEGACY_MARKERS: set[str] = {
    "pytest.mark.legacy",
    "pytest.mark.legacy_architecture",
}


def test_no_legacy_test_directories() -> None:
    for d in LEGACY_TEST_DIRS:
        p = PROJECT_ROOT / d
        assert not p.exists(), f"Legacy test directory '{d}' still exists"


def test_no_legacy_markers_in_test_code() -> None:
    violations: list[str] = []
    this_file = Path(__file__).resolve()
    test_dir = PROJECT_ROOT / "tests"
    for filepath in _get_python_files(test_dir):
        if filepath.resolve() == this_file:
            continue
        try:
            text = filepath.read_text(encoding="utf-8")
        except (OSError, UnicodeDecodeError):
        ...
        for marker in LEGACY_MARKERS:
            if marker in text:
                rel = filepath.relative_to(PROJECT_ROOT).as_posix()
                violations.append(f"  {rel}: contains '{marker}'")
    assert not violations, "Legacy test markers found:\n" + "\n".join(violations)


def test_no_legacy_imports_in_tests() -> None:
    """Test files must not import deleted legacy packages."""
    violations: list[str] = []
    test_dir = PROJECT_ROOT / "tests"
    for filepath in _get_python_files(test_dir):
        try:
        ...
        for imp in _all_imports(tree):
            for prefix in DELETED_LEGACY_IMPORTS:
                if imp.startswith(prefix):
                    rel = filepath.relative_to(PROJECT_ROOT).as_posix()
                    violations.append(f"  {rel}: imports '{imp}'")
    assert not violations, "Legacy imports found in tests:\n" + "\n".join(violations)
