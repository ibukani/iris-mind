...
def buffer() -> InputBuffer:
    return InputBuffer(session_id="test", timeout_ms=100, max_fragments=10)


def test_add_fragment_accumulates(buffer: InputBuffer) -> None:
...
def test_add_fragment_no_flush_until_final(buffer: InputBuffer) -> None:
    buffer.add_fragment("hello ", is_final=False)
...
def test_flush_via_is_final() -> None:
    results: list[str] = []
    buf = InputBuffer(session_id="s", timeout_ms=1000)
...
def test_flush_via_timeout() -> None:
    results: list[str] = []
    buf = InputBuffer(session_id="s", timeout_ms=50, max_fragments=10)
    buf.set_flush_callback(lambda sid, text: results.append(text))
    buf.add_fragment("hello", is_final=False)
    time.sleep(0.15)
...
def test_flush_via_max_fragments() -> None:
    results: list[str] = []
    buf = InputBuffer(session_id="s", timeout_ms=1000, max_fragments=3)
    buf.set_flush_callback(lambda sid, text: results.append(text))
...
def test_cancel_clears_and_stops_timer(buffer: InputBuffer) -> None:
    buffer.add_fragment("hello", is_final=False)
    buffer.cancel()
    assert buffer.fragment_count == 0
    assert buffer.accumulated_text == ""
...
def test_close_prevents_further_additions(buffer: InputBuffer) -> None:
    buffer.close()
...
def test_callback_receives_session_id() -> None:
    ...
    buf = InputBuffer(session_id="mysession", timeout_ms=1000)
...
def test_add_fragment_after_close(buffer: InputBuffer) -> None:
