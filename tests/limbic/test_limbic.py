...
def test_build_response_style_neutral() -> None:
    ...
    # 中立状態
    style = manager.build_response_style()
    assert style == ""


def test_build_response_style_joy() -> None:
    manager = LimbicManager(event_bus=None)
    # 快・興奮状態
...
def test_build_response_style_anger() -> None:
    manager = LimbicManager(event_bus=None)
    # 不快・興奮（イライラ）状態
    manager._emotion.valence = -0.8
    manager._emotion.arousal = 0.6
...
def test_build_response_style_sadness() -> None:
    manager = LimbicManager(event_bus=None)
    # 不快・鎮静（悲しい）状態
